...
| readiness-agent | skills/readiness-agent/ | check whether a local single-machine workspace can train or run inference now, explain what is missing, and optionally apply safe user-space fixes |
...
- **/readiness**: route to `readiness-agent` for pre-run workspace checks, missing-item analysis, and safe user-space readiness fixes
...
**Environment Setup:**
- **readiness-agent**: "train check", "inference check", "preflight", "workspace readiness", "can this repo train", "can this repo run inference", "can it run now", "check my environment", "what is missing before running", "check env", "before training", "before inference", "fix my training environment", "missing model", "missing dataset", "missing train.py"

**Instructions**:
 - Do not give direct answers without following the skill workflow
 - Route operator implementation work to `operator-agent`
 - Route single-machine pre-run training or inference checks, missing-item analysis, and safe user-space readiness fixes to `readiness-agent`
...
- **readiness-agent**: "train check", "inference check", "preflight", "readiness", "workspace", "before running", "before training", "before inference", "can it train", "can it run inference", "can it run now", "check my environment", "what is missing", "fix my environment"
